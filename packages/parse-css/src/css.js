 ...
 */
export function read_stylesheet(parser) {
 ...
 * @param {Parser} parser
 * @param {string} close
 * @returns {Array<CSS.Rule | CSS.Atrule>}
 */
...
function read_selector_list(parser, inside_pseudo_class = false) {
	...
	return e.unexpected_eof(parser.template.length);
}
...
function read_selector(parser, inside_pseudo_class = false) {
	...
	while (parser.index < parser.template.length) {
		...
		if (parser.eat('&')) {
			...
			const matcher = parser.read(REGEX_MATCHER) ?? null;
			...
			const flags = parser.read(REGEX_ATTRIBUTE_FLAGS) ?? null;
...
function read_declaration(parser) {
	...
	if (!value && !property.startsWith('--')) {
		e.css_empty_declaration(start, index);
	}
...
function read_value(parser) {
	...
	// If we're still inside a quoted string or url, that's an error
	if (quote_mark || in_url) {
