...
function add_const_declaration(state, id, expression, metadata) {
	...
	let has_await = metadata.expression.has_await;
	...
	if (metadata.promises_id) {
		...
		if (promise_stmt) {
			run.thunks.push(b.thunk(b.block([promise_stmt, b.stmt(assignment)]), has_await));
		} else {
			run.thunks.push(b.thunk(assignment, has_await));
		}
	} else {
