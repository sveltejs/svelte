...
Content inside component tags becomes a [snippet prop](snippets) called `children`. You cannot have a separate prop by that name.
...
In Svelte 4 syntax, every property (declared via `export let`) is bindable, meaning you can `bind:` to it. In runes mode, properties are not bindable by default: you need to denote bindable props with the [`$bindable`](runes#$bindable) rune.
...
This is not recommended, and is no longer possible in Svelte 5, where properties like `onclick` replace `on:click` as the mechanism for adding [event handlers](event-handlers).
