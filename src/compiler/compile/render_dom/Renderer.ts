...
export default class Renderer {
  ...
  * @param {string[]} names
    ...
    dirty(names, is_reactive_declaration = false) {
        ...
        const dirty = /** @type {| import('estree').Identifier
                    | import('estree').MemberExpression} */ ((is_reactive_declaration ? x `$$self.$$.dirty` : x `#dirty`));
        const get_bitmask = () => {
        ...
        return /** @type {any} */ ({
            ...
            get expression() {
                ...
                if (!bitmask.length) {
                    return /** @type {import('estree').BinaryExpression} */ (x `${dirty} & /*${names.join(', ')}*/ 0`);
                }
  ...
  * @param {string | import('estree').Identifier | import('estree').MemberExpression} node
    ...
    reference(node, ctx = '#ctx') {
        ...
        if (member !== undefined) {
            const replacement = /** @type {import('estree').MemberExpression} */ (x `/*${member.name}*/ ${ctx}[${member.index}]`);
            if (nodes[0].loc)
