...
import { is_name_contenteditable, get_contenteditable_attr, has_contenteditable_attr } from '../utils/contenteditable.js';
import { regex_dimensions, regex_starts_with_newline, regex_non_whitespace_character, regex_box_size } from '../../utils/patterns.js';
...
import { is_interactive_element, is_non_interactive_element, is_non_interactive_roles, is_presentation_role, is_interactive_roles, is_hidden_from_screen_reader, is_semantic_role_element, is_abstract_role, is_static_element, has_disabled_attribute, is_valid_autocomplete } from '../utils/a11y.js';
...
function is_valid_aria_attribute_value(schema, value) {
    switch (schema.type) {
        ...
        case 'idlist': // if list of ids, split each
            return (typeof value === 'string' &&
                value.split(regex_any_repeated_whitespaces).every(/** @param {any} id */ (id) => typeof id === 'string'));
        case 'tokenlist': // if list of tokens, split each
            return (typeof value === 'string' &&
                value
                    .split(regex_any_repeated_whitespaces)
                    .every(/** @param {any} token */ (token) => (schema.values || []).indexOf(token.toLowerCase()) > -1));
        default:
            return false;
    }
}
  ...
  * @param {import('../Component.js').default} component  *
    ...
    constructor(component, parent, scope, info) {
        ...
        if (info.name === 'svelte:element') {
            ...
            this.tag_expr = new Expression(component, this, scope, /** @type {import('estree').Literal} */ (string_literal(this.name)));
        }
        ...
        if (this.namespace !== namespaces.foreign) {
            ...
            if (this.name === 'textarea') {
                if (info.children.length > 0) {
                    const value_attribute = info.attributes.find(/** @param {any} node */ (node) => node.name === 'value');
                    if (value_attribute) {
            ...
            if (this.name === 'option') {
                ...
                const value_attribute = info.attributes.find(/** @param {any} attribute */ (attribute) => attribute.name === 'value');
                if (!value_attribute) {
                    info.attributes.push({
                        type: 'Attribute',
                        name: 'value',
 ...
 * @param {any} a
        ...
        info.attributes.forEach(/** @param {any} node */ (node) => {
            switch (node.type) {
                case 'Action':
                ...
                case 'Let': {
                    ...
                    l.names.forEach(/** @param {any} name */ (name) => {
    ...
    validate_attributes() {
        ...
        this.attributes.forEach(/** @param {any} attribute */ (attribute) => {
    ...
    validate_attributes_a11y() {
        ...
        attributes.forEach(/** @param {any} attribute */ (attribute) => attribute_map.set(attribute.name, attribute));
        handlers.forEach(/** @param {any} handler */ (handler) => handlers_map.set(handler.name, handler));
        attributes.forEach(/** @param {any} attribute */ (attribute) => {
            if (attribute.is_spread)
                return;
            const name = attribute.name.toLowerCase();
            // aria-props
            if (name.startsWith('aria-')) {
                ...
                if (value === 'false')
                    value = false;
                if (value !== null && value !== undefined && aria.has(/** @type {import('aria-query').ARIAProperty} */ (name))) {
            ...
            if (name === 'role') {
                ...
                if (typeof value === 'string') {
                    value
                        .split(regex_any_repeated_whitespaces)
                        .forEach(/** @param {import('aria-query').ARIARoleDefinitionKey} current_role */ (current_role) => {
                        ...
                        if (!has_disabled_attribute(attribute_map) &&
                            !is_hidden_from_screen_reader(this.name, attribute_map) &&
                            !is_presentation_role(current_role) &&
                            is_interactive_roles(current_role) &&
                            is_static_element(this.name, attribute_map) &&
                            !attribute_map.get('tabindex')) {
                            const has_interactive_handlers = handlers.some(/** @param {any} handler */ (handler) => a11y_interactive_handlers.has(handler.name));
                            if (has_interactive_handlers) {
                                component.warn(this, compiler_warnings.a11y_interactive_supports_focus(current_role));
                            }
                        }
                        // no-interactive-element-to-noninteractive-role
                        if (is_interactive_element(this.name, attribute_map) &&
                        ...
                        if (is_non_interactive_element(this.name, attribute_map) &&
                            is_interactive_roles(current_role) &&
                            !a11y_non_interactive_element_to_interactive_role_exceptions[this.name]?.includes(current_role)) {
        ...
        if (typeof role_value === 'string' && roles.has(role_value)) {
            ...
            const invalid_aria_props = new Set(aria.keys().filter(/** @param {any} attribute */ (attribute) => !(attribute in props)));
            const is_implicit = role_value && role === undefined;
            attributes
                .filter(/** @param {any} prop */ (prop) => prop.type !== 'Spread')
                .forEach(/** @param {any} prop */ (prop) => {
                if (invalid_aria_props.has(/** @type {import('aria-query').ARIAProperty} */ (prop.name))) {
                    component.warn(prop, compiler_warnings.a11y_role_supports_aria_props(prop.name, role_value, is_implicit, this.name));
                }
            });
        }
        // no-noninteractive-element-interactions
        if (!has_contenteditable_attr(this) &&
            !is_hidden_from_screen_reader(this.name, attribute_map) &&
            !is_presentation_role(role_static_value) &&
            ((!is_interactive_element(this.name, attribute_map) &&
                is_non_interactive_roles(role_static_value)) ||
                (is_non_interactive_element(this.name, attribute_map) && !role))) {
            const has_interactive_handlers = handlers.some(/** @param {any} handler */ (handler) => a11y_recommended_interactive_handlers.has(handler.name));
            if (has_interactive_handlers) {
        ...
        if (!has_dynamic_role &&
            ...
            const interactive_handlers = handlers
                ...
                .filter(/** @param {any} handlerName */ (handlerName) => a11y_interactive_handlers.has(handlerName));
            if (interactive_handlers.length > 0) {
                component.warn(this, compiler_warnings.a11y_no_static_element_interactions(this.name, interactive_handlers));
            }
        }
    }
    validate_special_cases() {
        ...
        if (this.name === 'a') {
            ...
            if (component.compile_options.legacy &&
                target_attribute &&
            ...
            if (required_attributes) {
                const has_attribute = required_attributes.some(/** @param {any} name */ (name) => attribute_map.has(name));
                if (!has_attribute) {
        ...
        if (this.name === 'label') {
            ...
            const has_input_child = (children) => {
                ...
                /** @param {any} child */
                (child) => child instanceof Element && (a11y_labelable.has(child.name) || child.name === 'slot'))) {
                    return true;
                }
        ...
        if (this.name === 'figure') {
            const children = this.children.filter(/** @param {any} node */ (node) => {
            ...
            const index = children.findIndex(/** @param {any} child */ (child) => ( /** @type {Element} */(child)).name === 'figcaption');
    ...
    validate_bindings_foreign() {
        this.bindings.forEach(/** @param {any} binding */ (binding) => {
    ...
    validate_content() {
        ...
        if (this.contains_a11y_label)
            return;
        if (this.bindings.some(/** @param {any} binding */ (binding) => ['textContent', 'innerHTML'].includes(binding.name)))
            return;
    ...
    validate_event_handlers() {
        const { component } = this;
        this.handlers.forEach(/** @param {any} handler */ (handler) => {
            ...
            handler.modifiers.forEach(/** @param {any} modifier */ (modifier) => {
    ...
    add_css_class() {
        ...
        if (class_attribute && !class_attribute.is_true) {
            if (class_attribute.chunks.length === 1 && class_attribute.chunks[0].type === 'Text') {
                ...
                ( /** @type {import('./shared/Node.js').default[]} */(class_attribute.chunks)).push(new Text(this.component, this, this.scope, /** @type {any} */ ({
                    type: 'Text',
                    data: ` ${id}`,
                    synthetic: true
                })));
            ...
            this.attributes.push(new Attribute(this.component, this, this.scope, /** @type {any} */ ({
                type: 'Attribute',
                name: 'class',
    ...
    get slot_template_name() {
        return /** @type {string} */ (this.attributes
            .find(/** @param {any} attribute */ (attribute) => attribute.name === 'slot')
            .get_static_value());
    }
    optimise() {
        attributes_to_compact_whitespace.forEach(/** @param {any} attribute_name */ (attribute_name) => {
            const attribute = this.attributes.find(/** @param {any} a */ (a) => a.name === attribute_name);
 ...
 * @param {any} chunk
    ...
    get can_use_textcontent() {
        return (this.is_static_content &&
            this.children.every(/** @param {any} node */ (node) => node.type === 'Text' || node.type === 'MustacheTag'));
    }
    get can_optimise_to_html_string() {
        const can_use_textcontent = this.can_use_textcontent;
        const is_template_with_text_content = this.name === 'template' && can_use_textcontent;
        return (!is_template_with_text_content &&
            !this.namespace &&
