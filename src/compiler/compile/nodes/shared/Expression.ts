...
export default class Expression {
                ...
                if (names) {
                    names.forEach(/** @param {any} name */ (name) => {
                        if (template_scope.names.has(name)) {
                            ...
                            template_scope.dependencies_for_name.get(name).forEach(/** @param {any} name */ (name) => {
            ...
            enter(node, parent) {
                ...
                if (node.type === 'Identifier' && is_reference(node, parent)) {
                    ...
                    if (function_expression) {
                        if (template_scope.names.has(name)) {
                            contextual_dependencies.add(name);
                            template_scope.dependencies_for_name.get(name).forEach(/** @param {any} dependency */ (dependency) => {
  ...
  * @param {import('estree').Node} node
                        ...
                        if (owner.type === 'Attribute' && owner.parent.name === 'slot') {
                            const dep_scopes = new Set(deps.map(/** @param {any} name */ (name) => template_scope.get_owner(name)));
