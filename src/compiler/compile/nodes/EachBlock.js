...
export default class EachBlock extends AbstractBlock {
  ...
  * @param {import('../Component.js').default} component  *
    ...
    constructor(component, parent, scope, info) {
        ...
        this.contexts.forEach(/** @param {any} context */ (context) => {
        ...
        if (this.has_animation) {
            this.children = this.children.filter(/** @param {any} child */ (child) => !isEmptyNode(child) && !isCommentNode(child));
            if (this.children.length !== 1) {
                const child = this.children.find(/** @param {any} child */ (child) => !!( /** @type {import('./Element.js').default} */(child)).animation);
