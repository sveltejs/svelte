 ...
 */
export default function create_module(program, format, name, banner, sveltePath = 'svelte', helpers, globals, imports, module_exports, exports_from) {
        ...
        declarations: [
            {
                ...
                id: {
                    type: 'ObjectPattern',
                    properties: globals.map(/** @param {any} g */ (g) => ({
                        type: 'Property',
                        method: false,
                        shorthand: false,
                        computed: false,
...
function esm(program, name, banner, sveltePath, internal_path, helpers, globals, imports, module_exports, exports_from) {
    const import_declaration = {
        ...
        specifiers: helpers.map(/** @param {any} h */ (h) => ({
    ...
    const exports = module_exports.length > 0 && {
        ...
        specifiers: module_exports.map(/** @param {any} x */ (x) => ({
            type: 'Specifier',
...
function cjs(program, name, banner, sveltePath, internal_path, helpers, globals, imports, module_exports, exports_from) {
        ...
        declarations: [
            {
                ...
                id: {
                    type: 'ObjectPattern',
                    properties: helpers.map(/** @param {any} h */ (h) => ({
                        type: 'Property',
                        method: false,
                        shorthand: false,
                        computed: false,
    ...
    const internal_globals = get_internal_globals(globals, helpers);
    const user_requires = imports.map(/** @param {any} node */ (node) => {
            ...
            declarations: [
                {
                    ...
                    id: node.specifiers[0].type === 'ImportNamespaceSpecifier'
                        ...
                        : {
                            type: 'ObjectPattern',
                            properties: node.specifiers.map(/** @param {any} s */ (s) => ({
                                type: 'Property',
                                method: false,
                                shorthand: false,
                                computed: false,
    ...
    const user_exports_from = exports_from.map(/** @param {any} node */ (node) => {
        const init = x `require("${edit_source(node.source.value, sveltePath)}")`;
        return node.specifiers.map(/** @param {any} specifier */ (specifier) => {
