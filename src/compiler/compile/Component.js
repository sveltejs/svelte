...
import { extract_ignores_above_position, extract_svelte_ignore_from_comments } from '../utils/extract_svelte_ignore.js';
...
export default class Component {
  ...
  * @param {import('../interfaces.js').Ast} ast
    ...
    constructor(ast, source, name, compile_options, stats, warnings) {
        ...
        this.walk_module_js();
        this.push_ignores(this.ast.instance
  ...
  * @param {import('estree').Node} node
            ...
            const referenced_globals = Array.from(this.globals, 
            /** @param {any}params_0 */
            ([name, alias]) => name !== alias.name && { name, alias }).filter(Boolean);
            if (referenced_globals.length) {
                this.helpers.set('globals', this.alias('globals'));
            }
            const imported_helpers = Array.from(this.helpers, /** @param {any}params_0 */ ([name, alias]) => ({
                name,
                alias
            }));
            create_module(program, format, name, banner, compile_options.sveltePath, imported_helpers, referenced_globals, this.imports, this.vars
                .filter(/** @param {any} variable */ (variable) => variable.module && variable.export_name)
                .map(/** @param {any} variable */ (variable) => ({
                name: variable.name,
            ...
            if (!js_sourcemap_enabled) {
                ...
                js.map = apply_preprocessor_sourcemap(sourcemap_source_filename, js.map, 
                /** @type {string | import('@ampproject/remapping').RawSourceMap | import('@ampproject/remapping').DecodedSourceMap} */ (compile_options.sourcemap));
            }
        }
        return {
            js,
 ...
 * @param {any} _value
  ...
  * @param {string} name
    ...
    get_vars_report() {
        ...
        return vars_report.map(/** @param {any} v */ (v) => ({
            name: v.name,
  ...
  * @private
        ...
        if (node.type === 'ExportNamedDeclaration') {
            ...
            if (node.declaration) {
                if (node.declaration.type === 'VariableDeclaration') {
                    node.declaration.declarations.forEach(/** @param {any} declarator */ (declarator) => {
                        extract_names(declarator.id).forEach(/** @param {any} name */ (name) => {
                            const variable = this.var_lookup.get(name);
                            ...
                            if (!module_script &&
                                ...
                                /** @type {any} */ (declarator), compiler_warnings.unused_export_let(this.name.name, name));
                            }
                        });
                    });
                }
                ...
                node.specifiers.forEach(/** @param {any} specifier */ (specifier) => {
                    ...
                    if (variable) {
                        ...
                        if (!module_script &&
                            ...
                            /** @type {any} */ (specifier), compiler_warnings.unused_export_let(this.name.name, specifier.exported.name));
                        }
                    }
                });
                return null;
    ...
    extract_javascript(script) {
        if (!script)
            return null;
        return script.content.body.filter(/** @param {any} node */ (node) => {
            if (!node)
                return false;
        ...
        walk(script.content, {
            ...
            enter(node) {
                if (node.type === 'LabeledStatement' && node.label.name === '$') {
                    component.warn(/** @type {any} */ (node), compiler_warnings.module_script_reactive_declaration);
                }
            }
        });
 ...
 * @param {any} node
    ...
    walk_instance_js_pre_template() {
        ...
        script.content.body.forEach(/** @param {any} node */ (node) => {
            ...
            if (expression.left.type === 'MemberExpression')
                return;
            extract_names(expression.left).forEach(/** @param {any} name */ (name) => {
 ...
 * @param {any} node
        ...
        sorted_globals.forEach(/** @param {any} name */ (name) => {
        ...
        if (is_reference(/** @type {import('is-reference').NodeWithPropertyDefinition} */ (node), /** @type {import('is-reference').NodeWithPropertyDefinition} */ (parent))) {
            ...
            if (name[0] === '$') {
                ...
                if (name[1] !== '$' &&
                    ...
                    if (!((regex_contains_term_function.test(parent.type) && prop === 'params') ||
  ...
  * @param {import('estree').Pattern | null} element
    ...
    hoist_instance_declarations() {
        ...
        for (let i = 0; i < body.length; i += 1) {
            const node = body[i];
            if (node.type === 'VariableDeclaration') {
                const all_hoistable = node.declarations.every(/** @param {any} d */ (d) => {
                    if (!d.init)
                        return false;
                    ...
                    if (this.var_lookup.get(name).reassigned)
                        return false;
                    if (this.vars.find(/** @param {any} variable */ (variable) => variable.name === name && variable.module)) {
                        return false;
                    }
                    return true;
                });
                if (all_hoistable) {
                    node.declarations.forEach(/** @param {any} d */ (d) => {
                        const variable = this.var_lookup.get(( /** @type {import('estree').Identifier} */(d.id)).name);
  ...
  * @param {import('estree').Node} node
    ...
    extract_reactive_declarations() {
        ...
        this.ast.instance.content.body.forEach(/** @param {any} node */ (node) => {
            const ignores = extract_svelte_ignore_from_comments(node);
            if (ignores.length)
                this.push_ignores(ignores);
            if (node.type === 'LabeledStatement' && node.label.name === '$') {
  ...
  * @param {import('estree').Node} node
                    ...
                    enter(node, parent) {
                        ...
                        if (node.type === 'AssignmentExpression') {
                            ...
                            extract_identifiers(left).forEach(/** @param {any} node */ (node) => {
                ...
                if (module_dependencies.size > 0 && dependencies.size === 0) {
                    ...
                    /** @type {any} */ (node.body), compiler_warnings.module_script_variable_reactive_declaration(Array.from(module_dependencies)));
                }
        ...
        unsorted_reactive_declarations.forEach(/** @param {any} declaration */ (declaration) => {
            declaration.assignees.forEach(/** @param {any} name */ (name) => {
                if (!lookup.has(name)) {
                    lookup.set(name, []);
                }
        ...
        const cycle = check_graph_for_cycles(unsorted_reactive_declarations.reduce(/**
 * @param {any} acc
         * @param {any} declaration
         */ (acc, declaration) => {
            declaration.assignees.forEach(/** @param {any} v */ (v) => {
                declaration.dependencies.forEach(/** @param {any} w */ (w) => {
        ...
        const add_declaration = (declaration) => {
            if (this.reactive_declarations.includes(declaration))
                return;
            declaration.dependencies.forEach(/** @param {any} name */ (name) => {
    ...
    check_if_tags_content_dynamic() {
        this.tags.forEach(/** @param {any} tag */ (tag) => {
...
function process_component_options(component, nodes) {
  ...
  * @param {any} attribute
    ...
    if (node) {
        node.attributes.forEach(/** @param {any} attribute */ (attribute) => {
  ...
  * @param {import('../interfaces.js').Attribute} attribute
                ...
                switch (name) {
                    ...
                    case 'customElement': {
                        ...
                        const tag = value[0].expression.properties.find(/** @param {any} prop */ (prop) => prop.key.name === 'tag');
                        if (tag) {
                        ...
                        if (props) {
                            ...
                            for (const property of ( /** @type {import('estree').ObjectExpression} */(props.value)).properties) {
                                ...
                                for (const prop of property.value.properties) {
                                    ...
                                    if (['reflect', 'attribute', 'type'].indexOf(prop.key.name) === -1 ||
                                        (prop.key.name === 'type' &&
                                            ...
                                            /** @type {string} */ (prop.value.value)) === -1) ||
